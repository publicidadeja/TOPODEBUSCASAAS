...
function displayResults(results) {
    const container = document.getElementById('competitors-list');
    container.innerHTML = results.map(competitor => `
        <div class="bg-gray-50 rounded-lg p-4 hover:shadow-md transition-all duration-300">
            ...
            ` : ''}
            <h4 class="font-semibold text-lg mb-2">${competitor.name}</h4>
            <p class="text-gray-600 text-sm mb-2">${competitor.vicinity}</p>
            ${competitor.rating ? `
                <div class="flex items-center mb-2">
                    <span class="text-yellow-400">★</span>
                    <span class="ml-1">${competitor.rating} (${competitor.user_ratings_total})</span>
                </div>
            ` : ''}
            ...
            ` : ''}
            <button onclick="showDetails('${competitor.place_id}')"
                    class="mt-2 px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 transition-colors w-full">
                Ver Detalhes
            </button>
        </div>
    `).join('');
    ...
    try {
        ...
        modal.innerHTML = `
            <div class="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6">
                <h3 class="text-xl font-semibold mb-4">${data.result.name}</h3>
                <!-- Add more details here -->
                <button onclick="this.closest('.fixed').remove()"
                        class="mt-4 px-4 py-2 bg-gray-200 rounded hover:bg-gray-300 transition-colors">
                    Fechar
                </button>
            </div>
        `;
...
document.getElementById('view-toggle').addEventListener('click', function() {
    const listView = document.getElementById('competitors-list');
    const mapView = document.getElementById('competitors-map');
    const listText = document.querySelector('.list-view-text');
    const mapText = document.querySelector('.map-view-text');
