...
<div class="bg-white rounded-xl p-6 shadow-sm hover:shadow-md transition-all duration-300 border border-gray-100 mb-8">
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h3 class="text-lg font-semibold text-gray-800">Análise da Concorrência</h3>
                <p class="text-sm text-gray-500">Principais concorrentes na sua região</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-lg border border-gray-200 hover:shadow-lg transition-all duration-300">
            <div class="relative">
                @if(isset($competitor['photos'][0]))
                    <img src="{{ $competitor['photos'][0] }}" 
                         alt="{{ $competitor['name'] }}" 
                ...
                @if(isset($competitor['rating']))
                <div class="absolute top-4 right-4 bg-white px-3 py-1 rounded-full shadow-md flex items-center space-x-1">
                    <svg class="w-4 h-4 text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                    </svg>
                    <span class="text-sm font-medium">{{ number_format($competitor['rating'], 1) }}</span>
                </div>
                @endif
            </div>

            <div class="p-6">
                <h4 class="text-lg font-semibold text-gray-800 mb-2">{{ $competitor['name'] }}</h4>
                
                <div class="space-y-3">
                    @if(isset($competitor['address']))
                    <div class="flex items-start space-x-3">
                        <svg class="w-5 h-5 text-gray-400 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/>
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/>
                        </svg>
                        <span class="text-sm text-gray-600">{{ $competitor['address'] }}</span>
                    </div>
                    @endif

                    @if(isset($competitor['phone']))
                    <div class="flex items-center space-x-3">
                    ...
                    @if(isset($competitor['website']))
                    <div class="flex items-center space-x-3">
                        <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9"/>
                        </svg>
                        <a href="{{ $competitor['website'] }}" target="_blank" class="text-sm text-blue-600 hover:underline">Website</a>
                    </div>
                    @endif

                    @if(isset($competitor['review_count']))
                    <div class="flex items-center space-x-3">
                        <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/>
                        </svg>
                        <span class="text-sm text-gray-600">{{ number_format($competitor['review_count']) }} avaliações</span>
                    </div>
                    @endif
                </div>

                @if(isset($competitor['distance']))
                <div class="mt-4 pt-4 border-t border-gray-100">
                    <span class="text-sm text-gray-500">
                        Distância: {{ number_format($competitor['distance'], 1) }} km
                    </span>
                </div>
                @endif
            </div>
